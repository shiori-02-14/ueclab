...
変更内容を短く書き、関連 Issue があればリンクしてください。データの追加・修正では**出典 URL** を PR に書いてください。
...
- 研究室の本文は追加しない。公開されている教員メールは `data/emails.csv` に追加してよい。学内限定の連絡先や未公開の個人情報は追加しない。写真は公式 `labs.json` の `image_path`（`https://www.uec.ac.jp/arc/images/`）だけを使い、画像ファイルはリポジトリに置かない
